...
> **Changelog (v4)**: New title and subtitle; added AI Hub context (previously models-only, now expanding to MCPs); removed aggregate count, tiers kept separate with confidence that more are coming; generalized future asset types (no specific names); softened "registers with" to "connects through" for Gateway accuracy; added visual separator formatting around image placeholders; updated SEO metadata for new title; removed all em dashes; removed competitor links.

> **SEO metadata**
> - **Meta title**: The MCP Catalog is here: discover, deploy and connect (53 chars)
> - **Meta description**: Red Hat OpenShift AI 3.4 introduces the MCP Catalog with MCP servers from Red Hat, partners and community that you can discover, deploy and manage on OpenShift. (159 chars)
